  ...
  def bubble_sort!(&prc)
    ...
    start.downto(0) do |i|
      (0...i).each do |j|
        x,y = self[j], self[j + 1]
        if block_given?
          if (prc.call(x, y) == 1) then swap(j,j + 1) end
        elsif (x > y) then swap(j,j + 1) end
      end
    end
    self
  end
...
def subwords(word, dictionary)
  arr_substring = substrings(word)
  arr_substring.select {|s| dictionary.include?(s)}.uniq
end
...
def concatenate(strings)
  strings.inject("") { |result, el| result += el }
end
