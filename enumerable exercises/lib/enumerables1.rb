...
def array_sum_with_index(arr)
  arr.each_with_index.inject(0) {|sum, (num, i)| sum + (num * i) }
end
...
def price_is_right(bids, actual_retail_price)
  bids.select { |x| x <= actual_retail_price }.sort.last
end
...
def at_least_n_factors(numbers, n)
  numbers.reduce([]) { |arr, num| (num_factors(num) >= n)? arr << num : arr }
end

def num_factors(number)
  (1..number).select {|x| number % x == 0}.length
end
...
def ordered_vowel_word?(word)
  word.downcase.scan(/[aeiou]/).each_cons(2).all? {|x,y| x <= y}
end
...
def products_except_me(numbers)
  first = lambda { |x| return numbers[0...x] }
  second = lambda { |x| return numbers[(x + 1)..-1] || [] }

  numbers.each_with_index.reduce([]) do |arr, (num, i)|
    arr << array_product(first.call(i) + second.call(i))
  end
end
