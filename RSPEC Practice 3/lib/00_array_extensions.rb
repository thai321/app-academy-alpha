...
class Array
  def my_uniq
    arr = []
    self.each { |x| arr << x if !arr.include?(x)}
    arr
  end
end
...
class Array
  def two_sum
    (0...length).to_a.combination(2).select { |i, j| (self[i] + self[j]) == 0 }
  end
end
...
class Array
  def median
    ...
    arr = self.sort
    return nil if empty?
    return (length.even?) ? (arr[mid - 1] + arr[mid])/2.0 : arr[mid]
  end
end
...
class Array
  def my_transpose
    result = []
    len_row = self[0].length # magic to work with all
    len_row.times do |i|     # rectangular dimension matrix!
      row_temp = []
      self.each { |row| row_temp << row[i] }
      result << row_temp
    end
...
# 2D array   [[2,3],[4,5] <-- inner-array] <-- outer-array
# 4 x 3 --> 3 x 4

#    3      {4 times : get the value correspond to an index from
#  times <= {each row (inner-array)
#           {add these element to a array and append to result
