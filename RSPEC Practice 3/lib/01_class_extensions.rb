...
class String
  def caesar(shift)
    start = 'a'.ord
    result = chars.map do |ch|
      start = (ch.upcase == ch) ? 'A'.ord : 'a'.ord
      (((ch.ord + shift - start) % 26) + start).chr
    end
    result.join
  end
end
...
class Hash
  def difference(other_hash)
    hash = {}
    other_hash.each { |k,v| hash[k] = v if !self[k] }
    each { |k,v| hash[k] = v if !other_hash[k] }
    hash
  end
end
...
class Fixnum
  To_char = {}
  (0..9).each { |x| To_char[x] = x.to_s }
  (0..5).each { |x| To_char[x + 10] = ('a'.ord + x).chr }
  ...
  def stringify(base)
    ...
    until num / (base ** pow) == 0
      remainder = (num / (base ** pow)) % base
      result = To_char[remainder] + result
      pow += 1
    end
